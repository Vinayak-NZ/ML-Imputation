...
Census.train.tidy <- Census.train[!Census.train$student==-9, !(names(Census.train) %in% drop_vars)]

Census.test.tidy <- Census.test[!Census.test$student==-9, !(names(Census.test) %in% drop_vars)]
...
Census.test.tidy.miss <- ohe.miss(dat=Census.test.tidy.miss, varlist=
                                    c('region','resident.type','fam.comp',
                                      ...
                                      'industry','econ.act','social.grade'))
...
trainS_v1 <- xgboost(
  data = dtrain, max_depth = 2, eta = 0.3, nthread = 2, nrounds = 100,
