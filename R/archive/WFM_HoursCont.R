...
drop_vars <- c("person.id", "hours.worked")

Census.train.tidy <- Census.train[!Census.train$hours.cont==-9, !(names(Census.train) %in% drop_vars)]

Census.test.tidy <- Census.test[!Census.test$hours.cont==-9, !(names(Census.test) %in% drop_vars)]
...
Census.test.tidy.miss <- ohe.miss(dat=Census.test.tidy.miss, varlist=
                                    c('region','resident.type','fam.comp',
                                      ...
                                      'ethnicity','religion','econ.act','occupation',
...
trainHC_v1 <- xgboost(
  data = dtrain, max_depth = 2, eta = 0.5, nthread = 2, nrounds = 200,
